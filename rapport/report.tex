...
\item First implement the \cite{van2016private} \info{I think that it'll be done before the end of Decembre}
...
\section{IP Problem}
\change{Show that it's unfeasible to hide small data by conventional deterministic encryption scheme or one way function}
...
By correlation \unsure{check if I don't find an article for that} we could get back the whole dataset but, it is intrinsically impossible to fully hide the IOC while still allowing a subscriber to evaluate the rule.\\
...
% stupid things
% when request we can also add the same number of random ips with the set, which would randomize the response 
...
\item encrypt the message (CAO) in (aes, ctr) but add a starting padding of 16 bit '\x00' (used to see if the decryption match )
...
Both system will be really similar to what I need, so I will only have to do few modifications ! \info{Attention, See how I can use the file with the MIT licence}



\improvement{Reporting of Sightings is needed ? }
\begin{itemize}
...
\item traitor tracking: \info{See implementation}
\item Salt: how to store it \info{See implementation}
