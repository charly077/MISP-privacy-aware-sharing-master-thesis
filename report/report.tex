...
This need to be used carefully as no verifications are made on the rule. Checking every new IOC if it already exists would have been really too time consuming.\improvement[inline]{Perhaps allow to do it in an argument}
...
In rules generation and matching algorithm, the KDF is the most costly function. For this raison, I've decided to analyse their behavior in function of the length of the initial value as well as the time complexity used (iterations or rounds).\\
\subsubsection{PBKDF2}
\todo[inline]{Add element}
...
\todo[inline]{Add Graphs: en fonction des iteration, en fonction du nb ip}
\todo[inline]{Add Interpretation}
\improvement[inline]{Explain the benchmark in for the results}
...
\improvement[inline]{Do the article use both as well ?}
