...
update from api\\
		=> Not using the pyMisp, it would have some advantage like beeing kept updated if the api is modified but it was really to heavy for what the only request I was using. => I've made myself the request with the requests python library.
update from sql\\

\subsection{Parsing logs}

\subsection{Multiprocessing for reading logs}
=> What can be parallelized => important and explain later in the schemes used
...
\subsection{Mysql and the Virtual MISP Training Machine}

\subsection{On URL normalization}
\info{je pense que l'article s'appelle comme ça !!! }
...
\info{Discussion on it}
\info{Pourquoi il avait fait sa propre implementation}
